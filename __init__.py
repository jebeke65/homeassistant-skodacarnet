...
def setup(hass, config):
    """Setup Skoda Carnet component"""
    interval = config[DOMAIN].get(CONF_SCAN_INTERVAL)
    data = hass.data[DATA_KEY] = SkodaData(config)

    from skodacarnet import Connection

    _LOGGER.debug("Creating connection to skoda carnet")
    ...
    # login to carnet
    _LOGGER.debug("Logging in to skoda carnet")
    connection._login()
    if not connection.logged_in:
        _LOGGER.warning('Could not login to Skoda carnet, please check your credentials')
    ...
    def update(now):
        """Update status from Skoda Carnet"""
        try:
            ...
            if not connection.logged_in:
                ...
                if not connection.update(request_data = False):
                    _LOGGER.warning("Could not query update from Skoda carnet")
                    return False
                else:
                    _LOGGER.debug("Updating data from Skoda carnet")
    ...
    _LOGGER.info("Starting Skodacarnet component")
